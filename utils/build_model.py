...
def build_model(opts, printer=print):
    #模型构建函数
    model = None
    if opts.model == 'bingli':#这里可以忽略掉，用不到病理数据。
...
def get_model_opts(parser):
    ...
    group.add_argument('--yx-model', default="YXModelOnly", type=str, help='Name of YingXiangModel')#YXModelv1不存在，改为默认YXModelOnly
